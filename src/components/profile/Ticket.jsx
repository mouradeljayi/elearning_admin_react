import {
  ...
  export const Ticket = () => {
    const [plainteData, setPlainteData] = useState({ content: "", sujet: "", etat: 0, user: "" });
    const { user, setUser } = useStateContext();
    const [message, setMessage] = useState('');
  
    useEffect(() => {
      axiosClient.get("/plainte/")
        .then(response => setPlainteData(response.data))
    ...
    const handleSubmit = (event) => {
      if (!plainteData.content || !plainteData.sujet) {
      ...
      formData.append("content", plainteData.content);
      formData.append("sujet", plainteData.sujet);
  
      formData.append("etat", 0);
      formData.append("user", user.id);
  
      axiosClient.post(`/plainte/`, formData)
        .then(() => {
      ...
      setPlainteData({ content: "", sujet: "", etat: 0, user: "" })
          ...
          <Container maxW="full" centerContent  >
            ...
            <Flex>
              <Box
                ...
                p={{ sm: 5, md: 5, lg: 10 }}>
                <Box p={1}>
                <Heading mb={5} bgGradient='linear(to-l, #ffd140, #2b6cb0)' bgClip='text' fontSize={{ base: '2xl', sm: '3xl' }}>
        Suggestions d'amélioration
      </Heading>
                  <Wrap spacing={{ base: 20, sm: 3, md: 5, lg: 20 }} mt={5}>
  
                    <WrapItem >
                      <Flex bg={useColorModeValue("white", "gray.700")} borderRadius="xl" boxShadow="xl"
                      >
                        <Box m={10} color="#0B0E3F" w={{ base: 'full', sm: 'full', md: 'full', lg: '500px' }} h="50%">
                          <Heading mb="5" color={useColorModeValue("#10316B", "white")} fontSize="sm">N'hésitez pas à nous contacter si vous avez des questions ou si vous avez besoin d'aide - nous sommes là pour vous!</Heading>
                          {message &&
                          ...
                          <VStack spacing={5}>
                            <FormControl id="sujet">
                              <FormLabel color={useColorModeValue("#10316B", "white")}> Sujet</FormLabel>
  
                              <Input borderColor="gray.300" type="text" value={plainteData.sujet} onChange={handleInputChange} name="sujet" size="md" placeholder='Ecrivez votre sujet' />
                            </FormControl>
                            <FormControl id="name">
                              <FormLabel color={useColorModeValue("#10316B", "white")}>Message</FormLabel>
                              <Textarea
                                value={plainteData.content}
