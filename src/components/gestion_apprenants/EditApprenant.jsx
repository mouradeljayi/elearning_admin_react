...
function EditApprenant() {
    ...
    useEffect(() => {
        axiosClient.get(`/apprenant/${apprenant.id}/image`)
            .then(response => {
                setAvatarUrl(response.request.responseURL);
            })
            .catch(error => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            const response = await axiosClient.put(`/password_update/${apprenant.id}/`, {
    ...
    const handleSubmit = async (event) => {
        ...
        formData.append("address", apprenant.address);
        // Send the updated user data to the API
        for (const [key, value] of formData.entries()) {
            console.log(key + ": " + value);
        }
        try {
            const response = await axiosClient.put(`/user_update/${apprenant.id}/`, formData);
                                ...
                                <FormControl id="img" >
                                    <Center>
                                        <label htmlFor="imageInput">
                                            ...
                                            />
                                            <Avatar boxSize={['100px', '150px', '200px', '250px']} src={apprenant.imageUrl || avatarUrl}>
                                                <AvatarBadge
